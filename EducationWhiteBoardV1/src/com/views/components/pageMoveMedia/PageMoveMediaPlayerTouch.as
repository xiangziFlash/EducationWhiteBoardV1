package com.views.components.pageMoveMedia 
{
	 ...
	 * @author wang
	 */
	public class PageMoveMediaPlayerTouch extends Sprite 
	{
		...
		public function PageMoveMediaPlayerTouch() 
		{
			initContent();
			initListener();
		}
		...
		{
			//_sp_con.addEventListener(MouseEvent.MOUSE_DOWN, onConDown);
			var temp:DirectionGesture=new DirectionGesture(150);
			temp.isLocal=false;
			OSMultiTouch.getInstance().enableGesture(_sp_con,temp,onDragSP);
			//var temp:DirectionGesture=OSMultiTouch.getInstance().getGestures(_sp_con)[0] as DirectionGesture;
		}
		
		private function onDragSP(e:DirectionEvent):void
		{
			// TODO Auto Generated method stub
			//trace(e.direction)
			switch(e.direction)
			{
				case DirectionGesture.LEFT:
				case DirectionGesture.LEFT_BOTTOM:
				case DirectionGesture.LEFT_TOP:
					gotoPage(pageID+1);
					break;
				case DirectionGesture.RIGHT:
				case DirectionGesture.RIGHT_BOTTOM:
				case DirectionGesture.RIGHT_TOP:
					gotoPage(pageID-1);
					break;
