package com.scxlib
{
	...
	public class MediaPeiLie extends Sprite
	{
		...
		private function initContent():void
		{
			...
			_mask.graphics.drawRect(0,0,100,ConstData.stageHeight - 340);
		...
		public function setMask(ww:Number,isRight:Boolean):void
		{
			...
			if(_isRight)
			{
				...
				_mask.graphics.drawRect(0,0,ww,ConstData.stageHeight - 340);
				_mask.graphics.endFill();
			}else{
				_mask.x = ww+122;
				_mask.graphics.clear();
				_mask.graphics.beginFill(0,0.3);
				_mask.graphics.drawRect(0,0,ConstData.stageWidth - 290 - ww, ConstData.stageHeight - 340);
				_mask.graphics.endFill();
			}
		...
		public function setArr(arr:Array):void
		{
			...
			for (var i:int = 0; i < _mediaVoArr.length; i++) 
			{
				...
				var row:int = Math.ceil((ConstData.stageHeight - 340) / 200);
				if(i%row==0){
					_spShu=new Sprite();
					_spShu.name="spShu_"+int(i/row);
					if(_isRight)
					{
						_spShu.x=int(i/row)*280;	
					}else{
						_spShu.x=ConstData.stageWidth - 290 -int(i/row)*280-145;	
					}
