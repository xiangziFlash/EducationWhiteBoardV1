package com.lylib.touch.manipulators
{
	...
	/**
	 * 拖拽缩放旋转手势，以触控点的中心为物体的缩放旋转中心 具有惯性
	 * 
	 * @author		刘渊
	 * @version	2.0.3.2011_2_16_beta
	 */
	public class MultiPointRotation extends BaseGesture
	{
...
//		private var _maxHeight:Number = 4096;
		private var _minScale:Number = 0.4;
		...
		private var _moCaLi:Number = 1.3;
		 ...
		 * @param bound				边界
		 */		
		public function MultiPointRotation(isDragable:Boolean = true, isRotatable:Boolean = true, isScaleable:Boolean = true, isInertialEnable:Boolean = true, isBringToFront:Boolean=true, bound:Rectangle = null)
		{
			super();
		...
		private function onTimer(e:TimerEvent):void
		{
			...
			//{
			if(obj==null)return;
				_startX = obj.transform.pixelBounds.left+obj.transform.pixelBounds.width*0.5;
				_startY =  obj.transform.pixelBounds.top+obj.transform.pixelBounds.height*0.5;
			//} 
		//	catch(error:Error) 
			//{
				//trace("timer報錯")
...
//				_angleDamping = d2/d1;
		...
		private function upDataXY(num1:Number,num2:Number,isBoo:Boolean):void
		{
			/*if(isBoo==false){
				return;
			}
		...
		{//trace("move")
			...
			upDataXY(obj.parent.globalToLocal(manager.getConterPoint()).x,obj.parent.globalToLocal(manager.getConterPoint()).y,true);
		}
		...
		override public function touchUpHandler(event:TouchEvent):void
		{
			if(manager.blobs.length == 0)
			{
				...
				_upX = obj.transform.pixelBounds.left+obj.transform.pixelBounds.width*0.5;
				_upY =  obj.transform.pixelBounds.top+obj.transform.pixelBounds.height*0.5;
				...
				{//trace("enterFrame")
					...
					obj.stage.addEventListener(Event.ENTER_FRAME,onObjFrame);
				}else{
...
//					//trace(_speedX,_speedY,"xyxyxy")
			...
			{ 
				...
				//_upX = obj.parent.globalToLocal(manager.getConterPoint()).x;
				//_upY = obj.parent.globalToLocal(manager.getConterPoint()).y;
		...
		private function onObjFrame(e:Event):void
		{
			if(obj==null){
				_timer.stop();
				obj.stage.removeEventListener(Event.ENTER_FRAME,onObjFrame);
				if(touchEndFunction!=null)
				{
					touchEndFunction(this.obj);
				}
...
//			}
		...
		override public function dispose():void
		{
			...
			{
				trace("MultiDragScaleRotate出错  297行",error.message);
			}
