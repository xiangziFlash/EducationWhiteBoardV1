...
package com.lylib.touch
{
	...
	public class OSMultiTouch
	{
		...
		private var _objTouchManagerDict:Dictionary;
		
		
		
		public function OSMultiTouch(s:S)
		{
			_objTouchManagerDict = new Dictionary();
		}
		 ...
		 * 为obj注册一个手势。
		 * @param obj						要注册手势的对象。
		 * @param gesture					要注册的手势。手势分两种类型：
		 * 									一种是EventGesture。
		 * 									EventGesture类型的手势在com.lylib.touch.gestures包内，
		 * 									它们是靠事件驱动的。当满足某个手势时，该手势就会派发对应的GestureEvent，
		 * 									并执行listener监听函数。
		 * 									另一种是Manipulators。
		 * 									Manipulators类型的手势在com.lylib.touch.manipulators包内，
		 * 									它不会派发任何事件，只会完成特定的操作，
		 * 									所以当注册这种类型的手势时不用写listener参数。
		 * @param listener					监听函数。
		 * @param isStopPropagation			是否阻止事件流，默认为阻止时间流。
		 * @return 
		 */
		public function enableGesture(obj:InteractiveObject, gesture:BaseGesture, listener:Function = null, isStopPropagation:Boolean = true) : BaseGesture
		{
			...
			if (_objTouchManagerDict[obj] != null)
			{
				touchManager = _objTouchManagerDict[obj];
			}
			if (touchManager == null)
			{
				touchManager = new TouchManager(obj);
				_objTouchManagerDict[obj] = touchManager;
			}
		...
		public function disableGesture(gesture:BaseGesture) : void
		{
			...
			else
			{
				...
				{
					_objTouchManagerDict[touchManager.obj] = null;
					delete _objTouchManagerDict[touchManager.obj];
		...
		{
			var touchManager:TouchManager;
			for (var obj:Object in _objTouchManagerDict) 
			{
				touchManager = _objTouchManagerDict[obj];
				if (touchManager != null)
				{
		...
		public function disableAllGesture() : void
		{
			...
			{ 
				...
				_objTouchManagerDict[obj] = null;
				delete _objTouchManagerDict[obj];
			}
		}
		...
		public function disableObjectGestureType(obj:InteractiveObject, type:Class):void
		{
			...
			var touchManager:TouchManager = _objTouchManagerDict[obj] as TouchManager;
		 ...
		 */
		public function get objTouchManagerDict():Dictionary
		{
			return _objTouchManagerDict;
		}
	}
}
